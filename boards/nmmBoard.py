...
class NineMensMorris(Board):
    ...
    def compute_state(counters, intersections):
        isects = [[[0 for z in range(3)] for y in range(3)] for x in range(3)]

        tempx = sorted(intersections, key=lambda p: p.x)
        middle = tempx[9:15]
        del tempx[9:15]

        for x in range(0, 18, 3):
            tempy = sorted(tempx[x:x+3], key=lambda p: p.y)
            for i in range(3):
                if x < 9:
                    col = 0
                    ring = x % 4
                ...
                isects[ring][i][col] = tempy[i]
        
        middle = sorted(middle, key=lambda p: p.y)
        isects[0][0][1] = middle[0]
        isects[1][0][1] = middle[1]
        isects[2][0][1] = middle[2]
        isects[2][2][1] = middle[3]
        isects[1][2][1] = middle[4]
        isects[0][2][1] = middle[5]
            ...
            for j in range(3):
                for k in range(3):
                    ...
                    for counter in counters:
                        if euclidean(counter[:2], isect.pos) < 30:
                            if counter[5] > 100:
